...
def start_reaper(
    ...
    agents: mapping agent_name -> agent_record
      agent_record MUST contain last_heartbeat_ts: float (unix seconds)
    ...
    def loop():
        while True:
            ...
            now = _now()
            with lock:
                # 1) classify stale / tombstone / delete
                to_delete = []
                for name, a in list(agents.items()):
                    meta: AgentReapMeta = a.get("_reap")
                    if meta is None:
                        meta = AgentReapMeta()
                        a["_reap"] = meta
                    ...
                    if last is None:
                        # If you ever have agents without heartbeat timestamp, treat as stale immediately.
                        last = 0.0

                    # ALIVE -> STALE
                    if meta.state == AGENT_ALIVE and (now - last) > STALE_AFTER_S:
                        meta.state = AGENT_STALE
                        meta.stale_since_ts = now
                        publish_event("agent.stale", {
                            "agent": name,
                        ...
                        for job_id, job in jobs.items():
                            if job.get("leased_by") == name:
                                # If your scheduler uses lease_expires_at to requeue,
                                # this is enough to make it eligible immediately.
                                job["lease_expires_at"] = 0.0
                                publish_event("lease.forced_expire", {
                                    "job_id": job_id,
                                    "agent": name,
                                    "reason": "agent_stale",
                                })
                    ...
                    if meta.state == AGENT_STALE and meta.stale_since_ts is not None:
                        if (now - meta.stale_since_ts) > TOMBSTONE_AFTER_S:
                            ...
                            publish_event("agent.tombstoned", {
                                "agent": name,
                ...
                for name in to_delete:
                    # If you have a delete endpoint that also cleans related state,
                    # call that function here instead. For minimal mode:
                    agents.pop(name, None)
                    publish_event("agent.deleted", {
                        "agent": name,
                        "now": now,
